...
class SapB1Client(ERPClient):
    ...
    def _login_and_get_session(self) -> tuple[requests.Session, str]:
        """
        ...
        if not settings.base_url:
            raise ERPClientError("SAP Service Layer base URL not configured in .env")
        ...
        try:
            ...
            raise ERPClientError(f"SAP Login failed: {exc}") from exc

        data = resp.json()
        session_id = data.get("SessionId", "")
        if not session_id:
            raise ERPClientError(f"SAP Login response did not contain SessionId. Response: {data}")
    ...
    def _upload_attachment_to_sap(self, session: requests.Session, attachment_info: dict[str, str]) -> int | None:
    ...
    def post_draft(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if not settings.posting_enabled:
            raise ERPClientError("SAP posting is disabled (set SAP_POSTING_ENABLED=true in .env)")

        # Step 1: Login
        session, session_id = self._login_and_get_session()
        ...
        try:
            ...
            if attachment_info:
                entry = self._upload_attachment_to_sap(session, attachment_info)
            ...
            raise ERPClientError(f"Failed to post to SAP PurchaseInvoices: {exc}") from exc
        finally:
            # Step 5: Logout
            self._logout(session)


    def _fetch_all_odata_pages(self, session: requests.Session, initial_url: str) -> list[dict[str, Any]]:
    ...
    def _fetch_sql_query_from_sap(self, session: requests.Session, sql_name: str, sql_text: str) -> list[dict[str, Any]]:
        ...
        return self._fetch_all_odata_pages(session, exec_url)
    ...
    def get_all_open_documents(self, doc_type: str = "PO") -> list[dict[str, Any]]:
        ...
        try:
            ...
            sql_text = f"""
                ...
                T0."CardCode", T0."CardName", T0."LicTradNum", T0."Series", T0."BPLId", T1."LineNum", T1."ItemCode", T1."Dscription",
                T1."Quantity", T1."PriceBefDi", T1."LineTotal", T1."VatSum", T1."WhsCode", T1."AcctCode",
                T1."VatGroup", T1."TaxCode", T1."OcrCode", T1."OcrCode2", T1."OcrCode3", T1."HsnEntry", T1."SacEntry", T1."LocCode"
                FROM {table_h} T0 
            ...
            flat_results = self._fetch_sql_query_from_sap(session, sql_name, sql_text)
            ...
            for row in flat_results:
                ...
                docs[doc_num]["DocumentLines"].append({
                    ...
                    "SacEntry": row.get("SacEntry") or row.get("HsnEntry"),
    ...
    def sync_all_master_data(self, target_category: str | None = None) -> list[dict[str, Any]]:
        """
        ...
        """
        session, _ = self._login_and_get_session()
        ...
        try:
            ...
            if not target or target == "vendors":
                try:
                    ...
                    bp_items = self._fetch_all_odata_pages(session, bp_url)
                    for b in bp_items:
                        code = b.get("CardCode")
                        if code:
            ...
            if not target or target == "wtax_codes":
                try:
                    ...
                    wt_list = self._fetch_all_odata_pages(session, wt_url)
                    for w in wt_list:
                        wcode = w.get("WTCode")
                        if wcode:
                            master_records.append({
            ...
            if not target or target == "items":
                try:
                    ...
                    item_list = self._fetch_all_odata_pages(session, item_url)
                    for i in item_list:
                        code = i.get("ItemCode")
                        if code:
                            master_records.append({
            ...
            if not target or target == "accounts":
                try:
                    ...
                    acct_list = self._fetch_all_odata_pages(session, acct_url)
                    if not acct_list:
            ...
            if not target or target == "tax_codes":
                try:
                    ...
                    tax_list = self._fetch_all_odata_pages(session, tax_url)
                    for t in tax_list:
                        code = t.get("Code")
                        if code:
                            master_records.append({
            ...
            if not target or target.startswith("cost_centers"):
                try:
                    ...
                    cc_list = self._fetch_all_odata_pages(session, cc_url)
                    for d in cc_list:
            ...
            if not target or target == "sac_entries":
                try:
                    ...
                    sac_list = self._fetch_all_odata_pages(session, sac_url)
                    for d in sac_list:
            ...
            if not target or target == "place_of_supply":
                try:
                    ...
                    states_list = self._fetch_all_odata_pages(session, states_url)
                    count = 0
            ...
            if not target or target == "locations":
                try:
                    ...
                    loc_list = self._fetch_all_odata_pages(session, loc_url)
                    count = 0
                    for l in loc_list:
    ...
    def fetch_historical_ap_invoices(self, top: int = 100) -> list[dict]:
        """Fetch historical posted AP Invoices (PurchaseInvoices) from SAP B1 Service Layer."""
        session, sid = self._login_and_get_session()
        try:
            base = settings.base_url.rstrip('/')
